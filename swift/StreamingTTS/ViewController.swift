...
class ViewController: NSViewController {
    ...
    private func setupUI() {
        ...
        let fastButton = NSButton(title: "Fast", target: self, action: #selector(speak))
        fastButton.bezelStyle = .rounded
        fastButton.font = NSFont.systemFont(ofSize: 16, weight: .bold)
        
        let latencyLabel = NSTextField(labelWithString: "Latency: --ms")
    ...
    @objc private func speak() {
        ...
        // Use faster rate for shorter latency
        let utterance = AVSpeechUtterance(string: text)
        utterance.rate = 0.5 // Default is 0.17, 0.5 is faster
